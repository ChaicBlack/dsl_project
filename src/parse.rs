...
impl Parse {
    ...
    /// Return the next entry as string.
    ///
    /// If the next entry cannot be represented as String, an error is returned.
    ...
    pub(crate) fn next_bytes(&mut self) -> Result<Bytes, ParseError> {
        match self.next()? {
            // Both 'Simple' and 'Bulk' may represent raw bytes.
            //
            // Although errors are stored as strings and could be represented as
            // raw bytes, they are considered two seperate types.
    ...
    pub(crate) fn next_int(&mut self) -> Result<u64, ParseError> {
        ...
        // This massage will be used twice, for simple and bulk.
        const MSG: &str = "protocol error; invalid number";

        match self.next()? {
            // An Integer frame is already stored as an int.
            Frame::Integer(val) => Ok(val),
            // Simple and bulk need to be parsed to int. If parsing fails,
            // an error is returned.
            Frame::Simple(data) => atoi::<u64>(data.as_bytes()).ok_or_else(|| MSG.into()),
            Frame::Bulk(data) => atoi::<u64>(&data).ok_or_else(|| MSG.into()),
            frame => Err(format!("protocol error; expexted int frame but got {:?}", frame).into()),
        }
    }

    /// Ensure there are no entries in the array.
